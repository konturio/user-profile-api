...
-- Assign the role to users who don't have any trial role in Risk Assessment app
WITH users_without_role AS (
    SELECT u.id AS user_id
    FROM users u
    WHERE NOT EXISTS (
        SELECT 1
        FROM user_custom_role ur
        JOIN custom_role cr ON cr.id = ur.role_id
        WHERE ur.user_id = u.id
          AND cr.name = 'risk_compass_trial'
    )
)
