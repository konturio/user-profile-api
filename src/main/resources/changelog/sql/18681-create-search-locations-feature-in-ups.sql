...
--changeset user-profile-service:18681-create-search-locations-feature-in-ups.sql runOnChange:false

--create search_locations feature
insert into feature ("name", featuretype, enabled, beta, available_for_user_apps, default_for_user_apps, description)
values ('search_locations', 'UI_PANEL', true, false, true, false, 'Search bar which allows to search locations in admin boundaries layer')
...
from feature f
where f.name in ('search_locations')
on conflict do nothing;
